...
type SectionRow = { id: string; type: SectionType; html: string; orderIndex: number };
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_360px]">
        <div>
          ...
          <div className="overflow-hidden rounded-lg border border-[var(--dg-border)] bg-white">
            <ScaledFrame srcDoc={design.fullHtml} frameWidth={device} frameHeight={1400} title={design.name} />
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          {sections.map((section) => (
            <div
              key={section.id}
              className="rounded-md border border-[var(--dg-border)] bg-white p-2.5"
            >
              ...
              <div className="overflow-hidden rounded border border-[var(--dg-border)]">
                <ScaledFrame
                  srcDoc={assembleStandaloneHtml(section.type, [section.html])}
                  frameWidth={1440}
                  frameHeight={220}
                  title={section.type}
                />
